  ...
  ':root'({data}) {
    return {
      prompts: `
        ...
        需求描述中所表达的输入均来自inputs,所有的返回均使用outputs下函数处理,如果需求描述中没有强调输入,那么不需要使用inputs,请根据需求按需正确使用函数入参inputs和outputs编写基于函数模版A的Javascript代码,不需要任何其它的解释或注释,以下是例子:
        问: 将输入的时间戳增加24小时
        ...
        问: 将输入加10输出
        ({outputs,inputs})=>{outputs.output0(inputs.inputValue0+10)}
        问: 加10输出
        ({outputs,inputs})=>{outputs.output0(10)}`,
