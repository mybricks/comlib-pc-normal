...
import { generateColumnsTitle, createColBySpan, getRowItem, getColIndex, getRowIndex, updateSlotTitle, updateCol } from './common'
// 添加行
export function addRow({ data, slot }: EditorResult<Data>, columnCount: number) {
    const columns: ColumnParams[] = [];
    const slotTitle = generateColumnsTitle({ widthOption: WidthUnitEnum.Span, span: 24 / columnCount });
  ...
  export function addColumn({ data, focusArea, slot }: EditorResult<Data>) {
    ...
    const title = generateColumnsTitle({ widthOption: WidthUnitEnum.Span, span: lastColumn.span || 4 });
  ...
  export function divideColumn({ data, focusArea, slot }: EditorResult<Data>) {
    ...
    const slotTitle = generateColumnsTitle({ widthOption: WidthUnitEnum.Span, span: 24 / item.columns.length });
