...
export default function ({ data, input, output, slot }: UpgradeParams<Data>): boolean {
  ...
  if (!slot.get('item').inputs?.get('originDataSource')) {
    slot.get('item').inputs?.add('originDataSource', '列表数据', {
      type: 'array',
      items: {
        title: '列项数据',
        type: 'any'
  ...
  if (!slot.get('item').inputs?.get('length')) {
    slot.get('item').inputs?.add('length', '列表数据个数', {
      type: 'number'
    });
  }
