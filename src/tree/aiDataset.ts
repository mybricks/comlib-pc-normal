...
export default defineDataSet((utils) => {
  ...
  result['显示连线'] = utils.options
    .switch()
    .map((item) => simpleTemplate('显示连线', item.label, { showLine: item.value }));

  const scrollHeight = utils.number.int({ min: 0, max: 100 }) + 'px';
  result['可滚动高度'] = [
    simpleTemplate('可滚动高度', scrollHeight, {
  ...
  result['标题超出省略'] = utils.options
    .switch()
    .map((item) => simpleTemplate('标题超出省略', item.label, { titleEllipsis: item.value }));

  const openDepth = utils.number.int({ min: -1, max: 20 });
  result['默认展开深度'] = [simpleTemplate('默认展开深度', openDepth, { openDepth })];

  result['自定义空状态图片'] = utils.options
    .switch()
    .map((item) => simpleTemplate('自定义空状态图片', item.label, { isImage: item.value }));
  ...
  result['使用静态数据源'] = utils.options
    .switch()
    .map((item) => simpleTemplate('使用静态数据源', item.label, { useStaticData: item.value }));
  ...
  result['过滤功能'] = selectedOptions.map((complex) =>
    simpleTemplate('过滤字段', complex.map((i) => i.label).join(','), {
      filterNames: complex.map((i) => i.value)
    })
  );

  result['勾选功能'] = [];
  for (let key in checkableMap) {
    result['勾选功能'].push(simpleTemplate('勾选功能', key, { checkable: checkableMap[key] }));
  }

  result['拖拽功能'] = [];
  for (let key in checkableMap) {
    result['拖拽功能'].push(simpleTemplate('拖拽功能', key, { checkable: checkableMap[key] }));
  }

  result['支持放置'] = [];
  for (let key in checkableMap) {
    result['支持放置'].push(simpleTemplate('支持放置', key, { checkable: checkableMap[key] }));
  }
  ...
  result['禁用勾选框'] = utils.options.switch().map((i) =>
    simpleTemplate('禁用勾选框', i.label, {
  ...
  result['父子节点勾选联动'].push(
    ...utils.options.switch().map((i) =>
      simpleTemplate('全部节点下的父子节点勾选联动', i.label, {
        disableCheckbox: i.value,
        checkable: true
      })
    )
  ...
  result['父子节点勾选联动'].push(
    ...utils.options.switch().map((i) =>
      simpleTemplate('自定义节点下的父子节点勾选联动', i.label, {
        disableCheckbox: i.value,
        checkable: 'custom'
      })
    )
