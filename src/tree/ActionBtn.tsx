...
import { SystemAttentionCircleFill } from "@ant-design/icons";
...
export default function ActionBtns({
    ...
    const deleteNode = (treeData: TreeData[], key: string) => {
        return treeData.map((item) => {
            if (item.key === key) {
                ...
                item.children = deleteNode(item.children, key).filter(def => !!def);
            }
            return item;
        });
    };
    ...
    const btnClick = (id) => {
        if (env?.runtime) {
            if (id === MODIFY_BTN_ID) {
                ...
                if (id === DELETE_BTN_ID) {
                    data.treeData = deleteNode(treeData, record.key).filter(def => !!def);
                }
    ...
    const confirm = () => {
        const text = hasChildren ? '确定删除节点"${title}"及其所有子节点吗？此操作不可恢复！' : '确定删除节点"${title}"吗？此操作不可恢复！';
        Modal.confirm({
            icon: <SystemAttentionCircleFill />,
