...
export default function ({ env, data, inputs, outputs, style }: RuntimeParams<Data>) {
  ...
  useEffect(() => {
    if (runtime) {
      inputs[InputIds.SetImgsSrc]((val, relOutputs) => {
        if (Array.isArray(val)) {
          data.src = val;
          relOutputs['setImgsSrcDone'](val);
        }
      });
    }
  }, []);
  ...
  useEffect(() => {
    inputs[InputIds.GetImgsSrc]((val, relOutputs) => {
      relOutputs['getImgsSrcDone'](data.src);
    });
  }, [data.src]);
  ...
  return (
    <div className={css.container + ' img-list'}>
      <Image.PreviewGroup>
        {(data?.src || []).map((src) => {
          return (
            <Image
