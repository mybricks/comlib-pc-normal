          ...
          items: [
            {
              ...
              type: 'Select',
              description:
                `遵循Antd表单Layout字段特性，默认为 “水平”。
水平布局：标题与表单项左右水平分布，标题可配置宽度；
垂直布局：标题与表单项垂直上下分布，标题在上方，表单项在下方，标题宽度自适应；
内联布局：标题与表单项左右水平分布，标题宽度自适应。`,
              options: [
        ...
        },
        {
          ...
          items: [
            ...
            {
              title: '显示冒号',
              description: '是否展示表单项标题的右侧中文冒号，默认展示，开启此选项后，所有表单项的标题文本不应包含冒号',
              type: 'Switch',
              value: {
                get({ data }: EditorResult<Data>) {
