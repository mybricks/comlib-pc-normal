...
使用多个表单容器还是一个表单容器？
任何情况下，先考虑使用多个容器嵌套多个表单的方式来实现，比如标题排列不一致，每个表单有卡片样式等等，都必须使用多个表单容器，而非一个大型表单。

重要约束(!IMPORTANT)：
尽可能小范围地使用表单：表单不含标题和样式配置，如果有卡片、表单标题等装饰性元素和样式，可以使用多个容器嵌套多个表单来实现；
...
重要约束(!IMPORTANT)：
- 尽可能小范围地使用表单：表单不含标题和样式配置，如果有卡片、表单标题等装饰性元素和样式，可以使用多个容器嵌套多个表单来实现；
- 样式限制：表单容器不负责卡片、外层标题等装饰样式，所以外层必须套一层布局来实现间距等装饰样式和元素。
- 禁止嵌套：表单容器不允许嵌套表单容器。
...
- 确定类型：
  ...
  - 选择「渲染模式」（编辑/只读）。
- 确定布局(关键)：
  - 设定表单项标题和内容排列：一个表单只能配置统一的垂直或水平配置。
...
//   ],
  ...
  `,
    ...
    ':child(mybricks.normal-pc.form-container/form-item)': {
      get title() {
        return ':child(form-item) 也就是表单容器的表单项时，可以通过对下方配置项的path添加一个:parent前缀来执行以下配置'
      },
      configs: [
        '表单项/显示标题',
        {
