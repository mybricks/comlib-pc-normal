...
使用流程：
  ...
  3. 配置布局相关信息，重点关注「表单项布局」和「表单项宽度配置」相关配置；
    其中「表单项布局」配置的是公共的布局，「表单项宽度配置」配置的是每个表单项的宽度，配置流程应该是先确认公共布局，再配置每个表单项的宽度。
    3.1 先配置「表单项布局」，一般选取最多的那一行的列数，比如一行三列，可以配置「表单项布局/每行列数」为*3*以及合理的列间距，同时配置「表单项宽度配置(共24格)」为*8*；
    3.2 特殊情况下，比如第二行只要一列，有两种方式：
      3.2.1 如果希望第四个表单项占满整行，则第四个表单项配置「表单项宽度配置(共24格)」为*24*；
      3.2.2 如果希望第四个表单项宽度和之前一样，则将第5第6个表单项用自定义表单项来占位；
