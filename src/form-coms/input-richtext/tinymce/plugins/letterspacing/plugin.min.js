 ...
 * Licensed under MIT
 */
myTinymce.PluginManager.add('letterspacing', function (a, l) {
  function g(b) {
    var e = a.dom,
    ...
    h.each(f, function (c) {
      if (e.getStyle(c, 'text-indent')) {
        ...
        c &&
          ...
          (d = (d = (d =
            c.children['0'].attributes.style.textContent.match(/font-size:?(.+?)"?[;}]/))
            ? d[1]
            : !1)
            ? 2 * (parseInt(d) + parseInt(b ? b : 0)) + 'px'
            : 2 * (parseInt(b ? b : 0) + 16) + 'px');
        e.setStyle(c, 'text-indent', d ? d : '2em');
      }
    });
  ...
  var h = myTinymce.util.Tools.resolve('myTinymce.util.Tools'),
    k = a.getParam('letterspacing', '0px 1px 2px 4px 6px 8px 10px 20px 40px');
  a.on('init', function () {
