...
const menuItemEditors = (props) => {
  ...
  if (getMenuItem(props, 'children') !== undefined) {
    for (let i = 0; i < getMenuItem(props, 'children').length; i++) {
      items.push({
        title: `${getMenuItem(props, 'children')[i].title}的点击事件`,
        type: '_Event',
...
export default {
  '@init': ({ data, output }: EditorResult<Data>) => {
    ...
    output.add(data.dataSource[0].key, '子菜单点击', schema);
  },
  '@resize': {
      ...
      value: {
        ...
        set({ data, output }: EditorResult<Data>, val: any[]) {
          ...
          for (let i = 0; i < [...val].length; i++) {
            output.add([...val][i].key, '子菜单点击', schema);
          }
        }
      }
    },
  ...
  '[data-menu-item]': (props: EditorResult<Data>, ...cateAry) => {
    (cateAry[0].title = '菜单项'),
      (cateAry[0].items = [
        {
          title: '标题',
          ...
          value: {
            ...
            set(props: EditorResult<Data>, value: any[]) {
              ...
              for (let i = 0; i < [...value].length; i++) {
                props.output.add([...value][i].key, `${[...value][i].title}的点击事件`, schema);
              }
            }
          }
        },
