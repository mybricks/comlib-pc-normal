...
export default function (props: RuntimeParams<Data>) {
  ...
  useEffect(() => {
    const target = ref.current?.querySelector?.(
      '.mybricks-table>div>div>div>div>div:nth-of-type(2)'
    ) as HTMLDivElement;
    ...
    if (target && data.fixedHeader && !!data.fixedHeight && runtime) {
    ...
    (ds: any) => {
      ...
      if (!usePagination && Array.isArray(ds)) {
        ...
        if (typeof ds?.pageSize === 'number' && ds?.pageSize > 0) {
          ...
          if (data.paginationConfig.currentPage) {
            data.paginationConfig.currentPage.pageSize = ds.pageSize;
          }
        }
        if (typeof ds?.pageNum === 'number') {
          data.paginationConfig.current = ds?.pageNum;
          if (data.paginationConfig.currentPage) {
            data.paginationConfig.currentPage.pageNum = ds.pageNum;
          }
        }
      } else {
        console.error('[数据表格]：未传入列表数据', ds);
                      ...
                      rowExpandable: (record) => {
                        ...
                        try {
                          isRowExpandable = data.isRowExpandable
                            ? eval(getTemplateRenderScript(data.isRowExpandable, false))(record)
                            : false;
                        } catch (e) {
