        ...
        set({ data, input, env }: EditorResult<Data>, val: string) {
          ...
          if (event?.options && event.options?.length > 1) {
            event.options.forEach((item, index) => {
              const pin = input.get(`input.${item.id}`);
              if (index === 0) {
                const defaultInput = data.eventInputs[0];
                defaultInput.key = item.id;
                input.get(InputIds.RUN).setTitle(item.title);
              } else {
                if (!pin) {
                  data.eventInputs.push({ id: item.id, key: item.id });
                  input.add({ id: `input.${item.id}`, title: item.title, schema: { type: 'any' } });
                }
              }
            });
            ...
            data.eventInputs.forEach((inputItem) => {
              ...
              if (!option && inputItem.id !== 'run') {
                const pin = input.get(`input.${inputItem.id}`);
                if (pin) {
                  input.remove(`input.${inputItem.id}`);
                }
              }
            });
          } else {
            data.eventInputs.forEach((item) => {
              if (item.id !== 'run') {
                const pin = input.get(`input.${item.id}`);
                if (pin) {
                  input.remove(`input.${item.id}`);
                }
              }
            });

            data.eventInputs = [{ id: 'run', key: event?.options[0].id }];
            input.get(InputIds.RUN).setTitle('触发事件');
          }
