    ...
    try {
      ...
      if (dynamicConfig) {
        ...
        finalOptions = {
          ...
          mockSchema: dynamicConfig.outputSchema
        };
      }
      ...
      env
        ...
        .then((val) => {
          if (
            curParams.showToplLog &&
            typeof val === 'object' &&
            val !== null &&
            val.__ORIGIN_RESPONSE__
          ) {
            (val.__ORIGIN_RESPONSE__?.logStack || []).map((log) => {
              ...
              logger.info(
                ` [${log.typeLabel}]  ${
                  log.nodeType === 'com' ? '组件' : log.nodeType === 'frame' ? '服务卡片' : 'Fx卡片'
                }${
                  log.type !== 'sql'
                    ? `：${log.title || log.id} | ${log.pinTitle || log.pinId}`
                    : ''
                }  当前值：${value}`,
...
export default function ({ env, data, inputs, outputs, logger }) {
  if (env.runtime) {
    if (data.immediate) {
      ...
      inputs[INPUT_ID.INPUT]((val: Record<string, any>) => {
        const { call: Params = {}, setUrl: url } = val;
        if (url && typeof url === 'string') {
          data.connectorConfig.url = url;
        }
        callCon(
          { env, data, outputs, logger },
